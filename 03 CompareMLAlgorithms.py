...
def compareBaseline(X, Y):
    ...
    baselineModels.append(('KNN', KNeighborsClassifier()))
    baselineModels.append(('LR', LogisticRegression(solver='newton-cg', multi_class = 'multinomial')))
    baselineModels.append(('NB', GaussianNB()))
    baselineModels.append(('RF', RandomForestClassifier()))
    baselineModels.append(('SGD', SGDClassifier()))
    baselineModels.append(('XGB',XGBClassifier(tree_method='gpu_hist')))
    ...
    for name, model in baselineModels:
        ...
        cvAccuracy = cross_val_score(model, X, Y, cv=kfold, scoring=scoring)
...
def main(transferLearning = "YES"):
    ...
    file = 'Data/accuracyList%sFE.txt' % transferLearning
